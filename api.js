...
const dataResolver = {
        ...
        axios.get(`https://restcountries.eu/rest/v2/alpha/${parameters['geo-country-code'][parameters['geo-country-code'].length - 1]['alpha-3'].toLowerCase()}`)
            .then(response => {
                ...
                if (typeOfData == 'currencies' || typeOfData == 'languages') {
                    ...
                    return { type: 'bot reply', result: dataForReturn }
                } else {
                    return { type: 'bot reply', result: response.data[typeOfData] }
                }
            })
            .catch(error => {
                console.log(error);
    ...
    currencyConvert: ({ parameters, resolvedQuery }) => {
        ...
        if (from && to && amount) {
            ...
            axios.get(url).then((res) => {
                ...
                return { type: 'bot reply', result: value.eq(1).text() + ' ' + to }
            }).catch((e) => {
                return { type: 'bot reply', result: 'Can not do this operation' }
            })
        } else {
            return { type: 'bot reply', result: 'Can not do this operation' }
        }
    ...
    globalStats: ({ parameters }) => {
        ...
        if (type == 'covid19') {
            axios.get(`https://www.worldometers.info/coronavirus/`).then((res) => {
                ...
                return { type: 'bot reply', result: `${value.eq(0).text()} total cases, ${value.eq(1).text()} deaths, ${value.eq(2).text()} recovered` }
            }).catch((e) => {
                return { type: 'bot reply', result: 'Can not do this operation' }
            })
            ...
            axios.get(`https://www.worldometers.info/world-population/`).then((res) => {
                ...
                return { type: 'bot reply', result: value.text() }
            }).catch((e) => {
                return { type: 'bot reply', result: 'Can not do this operation' }
            })
        }
    ...
    repeatText: ({ parameters }) => {
        return { type: 'bot reply', result: parameters.textForRepeat }
    },
    clearScreen: () => {
        return { type: 'bot do', result: 'clearScreen' }
    },
    changeTheme: () => {
        return { type: 'bot do', result: 'changeTheme' }
    }
}
    ...
    try {
        ...
        apiaiReq.on('response', (response) => {
            ...
            if (dataResolver[aiText]) {
                res.send(dataResolver[aiText](response.result))
            } else {
